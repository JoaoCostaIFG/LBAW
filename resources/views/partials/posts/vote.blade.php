...
<span class="row justify-content-center text-center {{ $size ?? 'fs-4' }}">
  <span class="col-12">
    <button id="btn-upvote-{{$post->id}}" @if($can_vote) onclick="updateVote({{$post->id}}, true)" @endif
     ...
     style="{{ $can_vote ? '' : 'cursor: not-allowed'}}" data-bs-toggle="tooltip" data-bs-placement="left" title="Upvote">
    </button>
  </span>
  ...
  <span class="col-12">
    <button id="btn-downvote-{{$post->id}}" @if($can_vote) onclick="updateVote({{$post->id}}, false)" @endif
     ...
     style="{{ $can_vote ? '' : 'cursor: not-allowed'}}" data-bs-toggle="tooltip" data-bs-placement="left" title="Downvote">
    </button>
  </span>
</span>

<div class="comment row gx-0 ms-1 py-2 pe-2 bg-dark border border-secondary">
  ...
  <div class="col-10 col-sm-11 row gx-0">
    ...
    <span class="col-12 d-flex gap-1 justify-content-end align-items-center text-break">
      <span class="text-muted">{{ (new \Carbon\Carbon($comment->post->date))->diffForHumans() }} by</span>
...
@auth @if (Auth::id() == $comment->post->owner->id || Auth::user()->hasRole('moderator'))
    <span>
      <button class="btn btn-sm btn-secondary" aria-label="Edit comment" aria-expanded="false">

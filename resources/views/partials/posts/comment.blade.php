<div class="comment row gx-0 ms-1 bg-dark border border-secondary align-items-center">
...
@else
  <span class="col-4 text-center">Deleted User</span>
@endif
  <p id="comment-{{$comment->post->id}}" class="col-7">{{ $comment->post->body }}</p>
  @auth
    @if (Auth::id() == $comment->post->owner->id || Auth::user()->hasRole('moderator'))
      <button id="commentEditDropdownLink" class="btn btn-secondary col-1 dropdown-toggle"
